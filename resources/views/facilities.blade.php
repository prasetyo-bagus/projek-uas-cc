...
@section('content')
    ...
    <!-- Facilities Grid -->
    <section class="py-20 bg-gray-50" x-data="{ isOpen: false, modalImage: '', modalTitle: '', modalDescription: '', modalDetail: '' }">
                        ...
                        <div
                            ...
                            <div class="relative overflow-hidden">
                                ...
                                <div
                                    ...
                                    <div class="p-5 translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                                        <span
                            ...
                            <div class="p-7">
                                ...
                                <!-- Detail Button -->
                                <button
                                    class="mt-2 inline-block px-4 py-2 bg-green-600 text-white text-sm font-semibold rounded-lg hover:bg-green-700 transition"
                                    @click="isOpen = true; modalImage = '{{ asset('storage/' . $facility->image) }}'; modalTitle = '{{ $facility->title }}'; modalDescription = `{{ e($facility->description) }}`; modalDetail = `{{ e($facility->detail) }}`">
                                    Detail
                                </button>
                            </div>
                        </div>
            ...
            @else
                <div class="text-center py-24">
                    <div class="inline-block p-8 rounded-2xl bg-white shadow-md border border-gray-100">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-16 h-16 mx-auto text-gray-400 mb-5" fill="none"
        ...
        <div x-show="isOpen" @keydown.window.escape="isOpen = false"
            ...
            x-cloak>
            <div class="bg-white rounded-xl overflow-hidden shadow-xl max-w-2xl w-full relative">
                <button @click="isOpen = false"
                    class="absolute top-2 right-2 text-white bg-black bg-opacity-50 rounded-full p-1 hover:bg-opacity-75">
                    ✕
                </button>
                <img :src="modalImage" alt="" class="w-full h-72 object-cover">
                <div class="p-6">
                    <h2 class="text-2xl font-bold text-gray-800" x-text="modalTitle"></h2>
                    <p class="text-gray-400 mb-1 leading-relaxed" x-text="modalDescription"></p>
                    <p class="text-gray-700" x-text="modalDetail"></p>
                </div>
            </div>
        </div>
