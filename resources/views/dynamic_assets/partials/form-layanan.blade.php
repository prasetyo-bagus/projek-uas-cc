...
<!-- Kolom Gambar -->
<div>
    <label for="image" class="block text-gray-700 font-medium mb-2">Gambar Layanan <span class="text-red-500">*</span></label>
    <input type="file" name="image" id="image" accept="image/*" {{ isset($asset) ? '' : 'required' }}
        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
    <p class="text-gray-500 text-xs mt-1">Ukuran gambar disarankan: 800x600px (rasio 4:3) untuk tampilan yang seragam, format: JPG, PNG</p>
    @error('image')
        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
    @enderror
</div>
