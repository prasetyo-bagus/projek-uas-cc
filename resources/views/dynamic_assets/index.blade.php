...
<div class="bg-white shadow-lg rounded-xl p-8 w-full mx-auto">
    <div class="flex items-center justify-between mb-8">
        ...
        <a href="{{ route('dynamic-assets.create') }}" class="bg-primary-600 hover:bg-primary-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors flex items-center">
    ...
    <div class="bg-gray-50 p-4 rounded-lg mb-6 flex flex-wrap gap-4 items-center">
        ...
        <div class="relative flex-grow md:max-w-xs">
            <input type="text" id="search-input" placeholder="Cari aset..." class="border border-gray-300 rounded pl-9 pr-3 py-1 w-full text-sm">
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200" id="assets-table-body">
                ...
                <tr class="asset-row" data-type="{{ $asset->type }}" data-status="{{ $asset->is_active ? 'active' : 'inactive' }}">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <img src="{{ asset('storage/' . $asset->image) }}" alt="{{ $asset->title }}" class="h-12 w-16 object-cover rounded">
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="px-2 py-1 text-xs rounded {{ 
                            $asset->type == 'BANNER' ? 'bg-blue-100 text-blue-800' : 
                            ($asset->type == 'GALERY' ? 'bg-purple-100 text-purple-800' : 
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $asset->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            <span class="w-2 h-2 mr-1 rounded-full {{ $asset->is_active ? 'bg-green-500' : 'bg-red-500' }}"></span>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex justify-end gap-2">
                            <form action="{{ route('dynamic-assets.toggle-status', $asset->id) }}" method="POST" class="inline">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 rounded-md p-1.5 transition-colors">
                                    <i class="fa-solid {{ $asset->is_active ? 'fa-toggle-on' : 'fa-toggle-off' }}"></i>
                                </button>
                            </form>
                            <a href="{{ route('dynamic-assets.edit', $asset->id) }}" class="text-amber-600 hover:text-amber-900 bg-amber-50 hover:bg-amber-100 rounded-md p-1.5 transition-colors">
                                <i class="fa-solid fa-pen-to-square"></i>
                            </a>
                            <form action="{{ route('dynamic-assets.destroy', $asset->id) }}" method="POST" class="inline delete-form">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-600 hover:text-red-900 bg-red-50 hover:bg-red-100 rounded-md p-1.5 transition-colors">
                ...
                <tr>
                    <td colspan="6" class="px-6 py-10 text-center text-gray-500">
                        ...
                        <a href="{{ route('dynamic-assets.create') }}" class="inline-block mt-3 text-primary-600 hover:text-primary-800">Tambah aset sekarang</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
