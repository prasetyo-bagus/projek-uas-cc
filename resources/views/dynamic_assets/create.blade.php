...
<div class="bg-white shadow-lg rounded-xl p-8 w-full mx-auto">
    <div class="flex items-center justify-between mb-8">
        ...
        <a href="{{ route('dynamic-assets.index') }}" class="flex items-center text-primary-600 hover:text-primary-800 transition-colors">
    ...
    <div class="mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <a href="{{ route('dynamic-assets.create', ['type' => 'BANNER']) }}" 
                ...
                <div class="w-16 h-16 flex items-center justify-center rounded-full {{ $type == 'BANNER' ? 'bg-primary-100 text-primary-600' : 'bg-gray-100 text-gray-500' }} mb-3">
            ...
            <a href="{{ route('dynamic-assets.create', ['type' => 'GALERY']) }}" 
                ...
                <div class="w-16 h-16 flex items-center justify-center rounded-full {{ $type == 'GALERY' ? 'bg-primary-100 text-primary-600' : 'bg-gray-100 text-gray-500' }} mb-3">
            ...
            <a href="{{ route('dynamic-assets.create', ['type' => 'FACILITY']) }}" 
                ...
                <div class="w-16 h-16 flex items-center justify-center rounded-full {{ $type == 'FACILITY' ? 'bg-primary-100 text-primary-600' : 'bg-gray-100 text-gray-500' }} mb-3">
                    <i class="fa-solid fa-building text-2xl"></i>
                </div>
                <h3 class="font-semibold {{ $type == 'FACILITY' ? 'text-primary-700' : 'text-gray-700' }}">Fasilitas</h3>
        ...
        <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
            <div class="flex items-center mb-6">
                <div class="w-10 h-10 flex items-center justify-center rounded-full bg-primary-100 text-primary-600 mr-3">
                    <i class="fa-solid {{ $type == 'BANNER' ? 'fa-image' : ($type == 'GALERY' ? 'fa-images' : 'fa-building') }} text-lg"></i>
                </div>
                <div>
                    <h3 class="text-xl font-semibold text-gray-800">Tambah {{ $type == 'BANNER' ? 'Banner' : ($type == 'GALERY' ? 'Galeri' : 'Fasilitas') }}</h3>
                    <p class="text-sm text-gray-500">Silakan isi detail informasi {{ strtolower($type == 'BANNER' ? 'Banner' : ($type == 'GALERY' ? 'Galeri' : 'Fasilitas')) }}</p>
                </div>
            </div>
            
            <form action="{{ route('dynamic-assets.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
    ...
    @else
        <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-8 text-center">
            ...
            <p class="text-gray-600 mb-4">Silakan pilih jenis aset terlebih dahulu dari pilihan di atas untuk menampilkan formulir.</p>
        </div>
    @endif
</div>
@endsection