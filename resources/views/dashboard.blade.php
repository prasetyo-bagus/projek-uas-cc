...
<div class="container mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        <p class="text-gray-600">Selamat datang di panel admin Nusantara Edupark.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500">
            ...
            <div class="mt-4 text-sm">
                <span class="text-blue-500">
                    <i class="fas fa-percentage"></i> {{ $totalBlogs > 0 ? round(($featuredBlogs / $totalBlogs) * 100) : 0 }}% dari total blog
                </span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-yellow-500">
            ...
            <div class="mt-4 text-sm">
                <span class="text-yellow-500">
                    <i class="fas fa-percentage"></i> {{ $totalAssets > 0 ? round(($facilities / $totalAssets) * 100) : 0 }}% dari total asset
                </span>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                @foreach($blogCategories as $category => $count)
                    ...
                    <div>
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-purple-600 h-2 rounded-full" style="width: {{ $totalBlogs > 0 ? ($count / $totalBlogs) * 100 : 0 }}%"></div>
                        </div>
                    </div>
                    @endif
                @endforeach
        ...
        <div class="bg-white rounded-lg shadow-md p-6 lg:col-span-2">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('blogs.index') }}" class="text-sm text-purple-600 hover:text-purple-800">Lihat Semua</a>
            </div>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <img src="{{ asset('storage/' . $blog->picture) }}" class="h-10 w-10 rounded-md object-cover" alt="{{ $blog->title }}">
                                    </div>
                                    @endif
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">{{ Str::limit($blog->title, 30) }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('dynamic-assets.index') }}" class="text-sm text-purple-600 hover:text-purple-800">Lihat Semua</a>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            @forelse($recentAssets as $asset)
            <div class="bg-gray-50 rounded-lg p-3">
                <img src="{{ asset('storage/' . $asset->image) }}" class="w-full h-32 object-cover rounded-md mb-2" alt="{{ $asset->title }}">
                <h3 class="text-sm font-medium text-gray-800 truncate">{{ $asset->title ?: 'Tanpa judul' }}</h3>
                <div class="flex justify-between items-center mt-2">
                    <span class="px-2 py-1 text-xs rounded-full {{ $asset->type == 'BANNER' ? 'bg-blue-100 text-blue-800' : ($asset->type == 'GALERY' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800') }}">
                        {{ $asset->type }}
                    </span>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            <a href="{{ route('blogs.create') }}" class="flex items-center p-4 bg-purple-100 rounded-lg hover:bg-purple-200 transition-colors">
            ...
            <a href="{{ route('dynamic-assets.create', ['type' => 'banner']) }}" class="flex items-center p-4 bg-blue-100 rounded-lg hover:bg-blue-200 transition-colors">
            ...
            <a href="{{ route('dynamic-assets.create', ['type' => 'galery']) }}" class="flex items-center p-4 bg-green-100 rounded-lg hover:bg-green-200 transition-colors">
            ...
            <a href="{{ route('dynamic-assets.create', ['type' => 'facility']) }}" class="flex items-center p-4 bg-yellow-100 rounded-lg hover:bg-yellow-200 transition-colors">
