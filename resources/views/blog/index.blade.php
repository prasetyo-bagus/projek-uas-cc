            ...
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                    ...
                    <tbody>
                            ...
                            <tr class="border {{ $blog->is_featured ?  : '' }}">
                                ...
                                <td class="py-2 px-4 border font-semibold text-blue-600">
                                    <a href="{{ route('blog.show', $blog->url) }}" 
                                       title="{{ $blog->title }}" 
                                       class="block">
                                       <span class="sm:hidden text-l">{{ \Illuminate\Support\Str::limit($blog->title, 20, '...') }}</span>
                                       <span class="hidden sm:inline text-l">{{ \Illuminate\Support\Str::limit($blog->title, 80, '...') }}</span>
                                    </a>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-3 px-6 text-center text-gray-500">Tidak ada blog tersedia.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div class="container mx-auto max-w-5xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($allBlogs as $blog)
                <div class="relative bg-white shadow-md overflow-hidden border border-gray-200 rounded-lg h-full flex flex-col">
                    <a href="{{ route('blog.show', $blog->url) }}" class="block hover:shadow-lg transition-shadow duration-200 h-full flex flex-col">
                        ...
                        @if($blog->category)
                            ...
                            <span class="absolute {{ $positionStyle }} left-2 {{ $categoryColor }} text-white text-xs px-2 py-1 rounded">
                                <p>{{ ucwords(str_replace('_', ' ', strtolower($blog->category))) }}</p>
                            </span>
                        @endif
                        
                        <div class="p-4 flex flex-col flex-grow">
                            <h2 class="font-semibold text-gray-800 hover:text-yellow-500 transition-colors duration-200 min-h-[80px]">
                                {{ $blog->title }}
                            </h2>
