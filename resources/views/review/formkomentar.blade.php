<section class="testimonial-section bg-gradient-to-r from-purple-900 to-purple-700 p-8 rounded-xl shadow-xl">
    <div class="container mx-auto">
        ...
        <form id="testimoniForm" action="{{ route('testimonials.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" onsubmit="return validateForm()">
                @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="nama" class="block text-white text-sm font-medium mb-2">Nama Anda <span class="text-red-300">*</span></label>
                    <div class="relative">
            ...
            <div>
                <label for="pesan" class="block text-white text-sm font-medium mb-2">Pesan Anda <span class="text-red-300">*</span></label>
                <div class="relative">
                    <div class="absolute top-3 left-3 flex items-start pointer-events-none">
                        <i class="fas fa-quote-left text-purple-300"></i>
                    </div>
                    <textarea name="pesan" id="pesan" placeholder="Bagaimana pengalaman Anda di Nusantara Edupark?" rows="5" required
            ...
            <div>
                <label class="block text-white text-sm font-medium mb-2">Rating <span class="text-red-300">*</span></label>
                <div class="star-rating flex flex-row-reverse justify-end space-x-3 space-x-reverse text-3xl">
                    <input type="radio" id="star5" name="rating" value="5" class="hidden">
                    <label for="star5" class="cursor-pointer text-gray-300 hover:text-yellow-400 transition-colors duration-200">★</label>
                    
                    <input type="radio" id="star4" name="rating" value="4" class="hidden">
                    <label for="star4" class="cursor-pointer text-gray-300 hover:text-yellow-400 transition-colors duration-200">★</label>
                    
                    <input type="radio" id="star3" name="rating" value="3" class="hidden">
                    <label for="star3" class="cursor-pointer text-gray-300 hover:text-yellow-400 transition-colors duration-200">★</label>
                    
                    <input type="radio" id="star2" name="rating" value="2" class="hidden">
                    <label for="star2" class="cursor-pointer text-gray-300 hover:text-yellow-400 transition-colors duration-200">★</label>
                    
                    <input type="radio" id="star1" name="rating" value="1" class="hidden">
                    <label for="star1" class="cursor-pointer text-gray-300 hover:text-yellow-400 transition-colors duration-200">★</label>
                </div>
            ...
            <div>
                ...
                <div class="mt-1 flex items-center">
                    <div id="photoPreview" class="hidden w-16 h-16 rounded-full bg-white/10 flex items-center justify-center mr-4 overflow-hidden">
                        <img id="previewImage" src="#" class="w-full h-full object-cover">
                    </div>
                    <label for="foto" class="cursor-pointer bg-white/10 hover:bg-white/20 py-2 px-4 border border-white/30 rounded-lg flex items-center transition-colors duration-200">
                        <i class="fas fa-camera mr-2 text-purple-300"></i>
                        <span class="text-white text-sm">Pilih Foto</span>
                        <input type="file" name="foto" id="foto" accept="image/*" class="hidden" onchange="previewPhoto(this)">
                    </label>
                </div>
